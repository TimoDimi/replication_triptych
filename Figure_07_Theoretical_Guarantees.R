...
df_A_X1 <- df_t %>%
  ...
  mutate(CEP = 4*t - 1.5,
         ...
         FAR = (2.5-4*t)^2) %>%
  bind_rows(tibble(t=c(0,1), HR=c(1,0), FAR=c(1,0))) %>%
  mutate(density=ifelse(((3/8 <= t) & (t <= 5/8)), 4, NA),
         density_max=max(density, na.rm = TRUE),
         density_rel=density/10,
         point_mass=0) %>%
  arrange(t, desc(HR), desc(FAR)) %>%
...
# Setting B
df_B_X1 <- df_t %>%
  mutate(CEP = ifelse((t<=0.25|t>=0.75), t, NA),
         ...
         FAR = ifelse(t<0.5,
                      ...
                      pmin((1-t)^2,1/16))) %>%
  bind_rows(tibble(t=c(0,1), HR=c(1,0), FAR=c(1,0))) %>%
  mutate(density=ifelse((t < 0.25 | t > 0.75), 1, NA),
         density_max=max(density, na.rm = TRUE),
         density_rel=density/10,
...
# Based on approximating the integral numerically
j_seq <- 0:3
Fj <- function(x,j) {pnorm(qnorm(x, sd = sqrt((j+1)/(4-j))))}


df_C <- df_t %>%
  bind_rows(tibble(t=c(0,1))) %>%
  arrange(t) %>%
  summarize(t = rep(t, each=length(j_seq)),
            j = rep(j_seq, n()),
  ...
  ungroup() %>%
  mutate(HR = 1 - 2*(t*CDF - Integral_hlp),
...
for (setting_choice in c("A", "B", "C")){
  ...
  p_ROC <- ggplot(df_ROC %>% filter(Setting==setting_choice)) +
  ...
  p_RelDiag <- ggplot(df_ROC %>% filter(Setting==setting_choice)) +
  ...
  # Joint plots together with patchwork
  ...
  ggsave(paste0("./plots/Fig07_", setting_choice, "_PopQuantities.pdf"),
         p_triptych,
